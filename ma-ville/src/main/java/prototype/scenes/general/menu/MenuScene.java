...
public class MenuScene extends Scenes {
    ...
    public void setScene() {
        ...
        if (!UserSession.getInstance().getUser().isIntervenant()){
            ...
            notificationBox.getChildren().addAll(this.notificationButton, this.notificationCountText);
            ...
            apiController.getNotifications(new FirebaseCallback<ArrayList<Notification>>() {
                ...
                public void onSuccessReturn(ArrayList<Notification> notifications) {
                    ...
                    Platform.runLater(() -> {
                        ...
                        // If there are notifications, add a red circle
                        if (notificationCount > 0) {
                            // Add red circle if there are notifications
                            Circle redCircle = new Circle(10, Color.RED);
                            redCircle.setTranslateX(10);
                            redCircle.setTranslateY(-10); // Adjust for proper position
                            notificationBox.getChildren().add(redCircle); // Add the circle to the notification box
                        } else {
                            // Remove red circle if no notifications
                            notificationBox.getChildren().removeIf(node -> node instanceof Circle);
                        }
