...
public class PreferencesPlagesHorairesScene extends Scenes {
    ...
    private VBox mainLayout; // VBox to stack the timetable and legend vertically
    ...
    public PreferencesPlagesHorairesScene(SceneController sceneController) {
        ...
        this.mainLayout = new VBox(20); // Add spacing between timetable and legend

        // Setting the title for the scene
        Text title = new Text("Préférences de Plages Horaires");
    ...
    private void setupGrid() {
        ...
        for (int i = 0; i < 7; i++) { // 7 days
            for (int j = 0; j < 13; j++) { // 13 time slots (08:00 to 20:00)
                ...
                int finalI = i;
                int finalJ = j;
                timeSlot.setOnMouseClicked(event -> toggleTimeSlot(timeSlot, finalI, finalJ));

                // Mark some slots as unclickable (in light red)
                if (shouldBeUnselectable(finalI, finalJ)) {
        ...
        redBox.setAlignment(Pos.CENTER); // Center items within the HBox
        Rectangle redSquare = new Rectangle(20, 20, Color.RED);
        redBox.getChildren().addAll(redSquare, new Text("Carré rouge : Occupé"));

        HBox whiteBox = new HBox(10);
        whiteBox.setAlignment(Pos.CENTER); // Center items within the HBox
        ...
        whiteBox.getChildren().addAll(whiteSquare, new Text("Carré blanc : Disponible"));
    ...
    private void toggleTimeSlot(Rectangle timeSlot, int day, int hour) {
